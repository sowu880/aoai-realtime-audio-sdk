...
async def run(client: RTClient,  user_message_file_path: str, out_dir: str):
    with open(user_message_file_path, encoding="utf-8") as user_message_file:
        instructions = f"Response to the user with the EXACT same input text that the user provides, but in your voice."
...
async def with_azure_openai( user_message_file_path: str, out_dir: str):
    ...
    async with RTClient(url=endpoint, key_credential=AzureKeyCredential(key), azure_deployment=deployment) as client:
        await run(client,  user_message_file_path, out_dir)
...
if __name__ == "__main__":
    load_dotenv()
    emotions = [ "neutral"]
    user_message_file_path = f"./sample_script.txt"
    out_dir = f"./output"
    provider = "azure"
    ...
    if provider == "azure":
        asyncio.run(with_azure_openai( user_message_file_path, out_dir))
    else:
